...
async function Validator() {
  let { data, error } = await supabase.from("Admin_Users").select();
  ...
  if (data) {
    ...
    const Checked_Email = Admin_Users.value
      .filter((e) => e.admin_email == email.value)
      .map((e) => {
        return { email: e.admin_email };
      });
